"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ContractDeployer = void 0;
const ethers_1 = require("ethers");
const config_1 = require("../config");
const logger_1 = require("../utils/logger");
class ContractDeployer {
    provider;
    rpcUrls;
    currentRpcIndex = 0;
    constructor() {
        this.rpcUrls = [
            config_1.config.rome.rpcUrl,
            config_1.config.rome.martisRpcUrl,
            'https://caelian.testnet.romeprotocol.xyz/',
            'https://martis.testnet.romeprotocol.xyz/'
        ];
        this.provider = new ethers_1.ethers.JsonRpcProvider(this.rpcUrls[0]);
    }
    async switchRpc() {
        this.currentRpcIndex = (this.currentRpcIndex + 1) % this.rpcUrls.length;
        this.provider = new ethers_1.ethers.JsonRpcProvider(this.rpcUrls[this.currentRpcIndex]);
        logger_1.logger.info(`Switched to RPC: ${this.rpcUrls[this.currentRpcIndex]}`);
    }
    async validateConnection() {
        try {
            const network = await this.provider.getNetwork();
            logger_1.logger.info(`Connected to network: ${network.name} (${network.chainId})`);
            return true;
        }
        catch (error) {
            logger_1.logger.error('Failed to connect to Rome network', error);
            return false;
        }
    }
    getHelloWorldContract() {
        // HelloWorld.sol contract with pragma 0.8.20
        const abi = [
            {
                "inputs": [],
                "name": "greet",
                "outputs": [
                    {
                        "internalType": "string",
                        "name": "",
                        "type": "string"
                    }
                ],
                "stateMutability": "view",
                "type": "function"
            },
            {
                "inputs": [
                    {
                        "internalType": "string",
                        "name": "_greeting",
                        "type": "string"
                    }
                ],
                "name": "setGreeting",
                "outputs": [],
                "stateMutability": "nonpayable",
                "type": "function"
            },
            {
                "inputs": [],
                "stateMutability": "nonpayable",
                "type": "constructor"
            }
        ];
        // Compiled bytecode for HelloWorld contract
        const bytecode = "";
        return { abi, bytecode };
    }
    async deployHelloWorld(walletPair) {
        let attempts = 0;
        const maxAttempts = this.rpcUrls.length;
        while (attempts < maxAttempts) {
            try {
                const wallet = new ethers_1.ethers.Wallet(walletPair.evm.privateKey, this.provider);
                // Check balance first
                const balance = await wallet.provider.getBalance(wallet.address);
                if (balance === 0n) {
                    throw new Error('Insufficient balance for deployment');
                }
                logger_1.logger.info(`Deploying HelloWorld contract from ${wallet.address} with balance ${ethers_1.ethers.formatEther(balance)} ETH`);
                const { abi, bytecode } = this.getHelloWorldContract();
                // Create contract factory
                const factory = new ethers_1.ethers.ContractFactory(abi, bytecode, wallet);
                // Estimate gas
                const estimatedGas = await factory.getDeployTransaction().then(tx => wallet.provider.estimateGas(tx));
                // Get gas price
                const feeData = await wallet.provider.getFeeData();
                const gasPrice = feeData.gasPrice || ethers_1.ethers.parseUnits('20', 'gwei');
                logger_1.logger.info(`Estimated gas: ${estimatedGas}, Gas price: ${ethers_1.ethers.formatUnits(gasPrice, 'gwei')} gwei`);
                // Deploy contract
                const contract = await factory.deploy({
                    gasLimit: estimatedGas + 50000n, // Add some buffer
                    gasPrice: gasPrice
                });
                // Wait for deployment
                const receipt = await contract.waitForDeployment();
                const address = await contract.getAddress();
                logger_1.logger.info(`HelloWorld contract deployed at: ${address}`);
                logger_1.logger.info(`Transaction hash: ${contract.deploymentTransaction()?.hash}`);
                // Verify deployment by calling greet function
                try {
                    const greeting = await contract.greet();
                    logger_1.logger.info(`Contract greeting: ${greeting}`);
                }
                catch (e) {
                    logger_1.logger.warn('Could not verify contract deployment by calling greet()');
                }
                return {
                    address: address,
                    hash: contract.deploymentTransaction()?.hash || '',
                    success: true
                };
            }
            catch (error) {
                logger_1.logger.error(`Deployment attempt ${attempts + 1} failed`, error);
                attempts++;
                if (attempts < maxAttempts) {
                    await this.switchRpc();
                    await new Promise(resolve => setTimeout(resolve, 2000));
                }
            }
        }
        return {
            address: '',
            hash: '',
            success: false,
            error: `Failed to deploy after ${maxAttempts} attempts`
        };
    }
    async interactWithContract(contractAddress, walletPair, newGreeting) {
        try {
            const wallet = new ethers_1.ethers.Wallet(walletPair.evm.privateKey, this.provider);
            const { abi } = this.getHelloWorldContract();
            const contract = new ethers_1.ethers.Contract(contractAddress, abi, wallet);
            if (newGreeting) {
                // Call setGreeting
                const tx = await contract.setGreeting(newGreeting, {
                    gasLimit: 100000,
                    gasPrice: ethers_1.ethers.parseUnits('20', 'gwei')
                });
                const receipt = await tx.wait();
                logger_1.logger.info(`Updated greeting. Transaction: ${receipt.hash}`);
            }
            // Call greet to read current greeting
            const greeting = await contract.greet();
            logger_1.logger.info(`Current greeting: ${greeting}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error('Failed to interact with contract', error);
            return false;
        }
    }
    async deployMultipleContracts(walletPairs, count = 1) {
        const results = new Map();
        for (const walletPair of walletPairs) {
            const walletResults = [];
            for (let i = 0; i < count; i++) {
                try {
                    logger_1.logger.info(`Deploying contract ${i + 1}/${count} for wallet ${walletPair.evm.address}`);
                    const result = await this.deployHelloWorld(walletPair);
                    walletResults.push(result);
                    // Add delay between deployments
                    await new Promise(resolve => setTimeout(resolve, 5000));
                }
                catch (error) {
                    logger_1.logger.error(`Failed to deploy contract ${i + 1} for wallet ${walletPair.evm.address}`, error);
                    walletResults.push({
                        address: '',
                        hash: '',
                        success: false,
                        error: error instanceof Error ? error.message : String(error)
                    });
                }
            }
            results.set(walletPair.evm.address, walletResults);
        }
        return results;
    }
    async getContractCode(address) {
        try {
            return await this.provider.getCode(address);
        }
        catch (error) {
            logger_1.logger.error(`Failed to get contract code for ${address}`, error);
            return '0x';
        }
    }
    async verifyContract(address) {
        try {
            const code = await this.getContractCode(address);
            return code !== '0x';
        }
        catch (error) {
            logger_1.logger.error(`Failed to verify contract ${address}`, error);
            return false;
        }
    }
    async estimateDeploymentCost() {
        try {
            const { abi, bytecode } = this.getHelloWorldContract();
            // Create a temporary wallet for estimation
            const tempWallet = ethers_1.ethers.Wallet.createRandom().connect(this.provider);
            const factory = new ethers_1.ethers.ContractFactory(abi, bytecode, tempWallet);
            const deployTransaction = await factory.getDeployTransaction();
            const gasEstimate = await this.provider.estimateGas(deployTransaction);
            const feeData = await this.provider.getFeeData();
            const gasPrice = feeData.gasPrice || ethers_1.ethers.parseUnits('20', 'gwei');
            const cost = gasEstimate * gasPrice;
            const costInEth = ethers_1.ethers.formatEther(cost);
            logger_1.logger.info(`Deployment cost estimate: ${gasEstimate} gas, ${costInEth} ETH`);
            return { gasEstimate, costInEth };
        }
        catch (error) {
            logger_1.logger.error('Failed to estimate deployment cost', error);
            return { gasEstimate: 0n, costInEth: '0' };
        }
    }
}
exports.ContractDeployer = ContractDeployer;
//# sourceMappingURL=ContractDeployer.js.map